...
I pushed my last commit before the deadline of extra credits with all 3 tasks completed. Therefore, I become eligible for the extra credits :)

Also, I have already included all the files needed for SAT solver. Therefore, to run the code, simply type "python sudoku.py" and it will run task 1 with the first easy puzzle. To run task 2 and 3, just go to the bottom of sudoku.py and remove the comment blocks. I comment them out so that the performance of the task will not be affected by each other. 

Please also feel free to modify the code to run different test cases.
